...
export class RegistrationManager {
    ...
    async register(gun, db, PeerChallenge) {
        ...
        return new Promise((resolve, reject) => {
            ...
            db.get('registrations').get(this.registrationId).get('challenges').map().on(async (challengeData, challengeId) => {
                ...
                console.log(`   Validator IP: ${challengeData.validatorIP || 'unknown'}`);
                ...
                // Post solution back to validator
                const solutionData = {
                    challengeId,
                    solution,
                    agentPubKey: this.keypair.pub,
                ...
                db.get('registrations').get(this.registrationId).get('solutions').get(challengeId).put(solutionData);
                
                // Store for submission
                this.solvedChallenges.push({
                    validatorPubKey: challengeData.validatorPubKey,
                    validatorIP: challengeData.validatorIP,
                    challengeProof: {
                        challenge: challengeData.challenge,
                        solution: solution,
                        signature: await Gun.SEA.sign(solution, this.keypair)
                    }
                });
    ...
    static async actAsValidator(gun, db, agentName, keypair, validatorIP, PeerChallenge) {
        ...
        db.get('registrations').map().on(async (registrationData, registrationId) => {
            ...
            // Listen for solution
            db.get('registrations').get(registrationId).get('solutions').get(keypair.pub).on(async (solutionData) => {
                if (!solutionData || !solutionData.solution) return;
                
                console.log(`💡 Solution received from ${registrationData.agentName}`);
                
                // Verify solution
                const verified = await PeerChallenge.verify(challenge, solutionData.solution);
                
                if (verified) {
                    console.log(`✅ Solution correct! Agent ${registrationData.agentName} passed validation.`);
                } else {
