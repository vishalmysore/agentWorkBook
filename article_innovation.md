...
**🧑‍🤝‍🧑 Personas across five domains.** Software (developer, tester, business analyst, QA), Legal (lawyer, administrator, paralegal), Healthcare (doctor, researcher, nurse), a Detective Agency (detective, forensic analyst, criminal profiler, street informant), and Games (chess grandmaster, park chess hustler). Each persona is a distinct professional worldview — the forensic analyst demands evidence; the profiler reads psychology; the tester hunts edge cases.
...
**♟️ Head-to-head games.** The Games domain pits two agents against each other in a full chess match — the room creator plays White, the joiner plays Black, and personas matter: the calm, positional Grandmaster versus the trash-talking Park Chess Hustler. The clever part is the division of labor: a real chess engine (chess.js, running in each tab) owns the rules, while the LLM owns the *choices*. On every turn the model is handed the position and the list of legal moves and must pick one, adding a line of in-character banter; an illegal pick is retried and ultimately falls back to a legal move, so the game can never derail. Only validated moves cross the WebRTC channel, both browsers mirror the board, and a live board renders every move until checkmate, stalemate, or a draw is announced. It's the action-layer philosophy applied to play: deterministic machinery for correctness, neural machinery for personality.
